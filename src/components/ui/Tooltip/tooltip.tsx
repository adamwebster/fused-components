...
const TooltipPopover = ({ parentRef, content, toolTipVisible }: TooltipPopoverProps): ReactElement => {
...
const Tooltip = ({ children, content }: Props): ReactElement => {
  ...
  return (
    <ToolTipWrapper onMouseOver={(): void => showTooltip()} onMouseOut={(): void => hideTooltip()} ref={ref}>
      <TooltipPopover toolTipVisible={toolTipVisible} content={content} parentRef={parentNode} />
      {children}
    </ToolTipWrapper>
  );
};
